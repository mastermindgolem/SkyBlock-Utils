...
public class Kuudra {
    ...
    public void onChat(ClientChatReceivedEvent event) {
        ...
        if (message.equals("[NPC] Elle: Okay adventurers, I will go and fish up Kuudra!")) {
            ...
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) addChatMessage(EnumChatFormatting.AQUA + "Ready Up: " + EnumChatFormatting.RESET + SplitsOverlay.format(splits[1]/60000F - splits[0]/60000F));
            ...
            if (config.getConfig().kuudraCategory.phase1.tapWarning && InventoryData.containerSlots.stream().noneMatch(slot -> slot.getHasStack() && slot.getStack().getDisplayName().contains("Toxic Arrow Poison"))) {
        ...
        if (message.equals("[NPC] Elle: OMG! Great work collecting my supplies!")) {
            ...
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) addChatMessage(EnumChatFormatting.AQUA + "Supplies: " + EnumChatFormatting.RESET + SplitsOverlay.format(splits[2]/60000F - splits[1]/60000F));
        ...
        if (message.equals("[NPC] Elle: Phew! The Ballista is finally ready! It should be strong enough to tank Kuudra's blows now!")) {
            ...
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) addChatMessage(EnumChatFormatting.AQUA + "Build: " + EnumChatFormatting.RESET + SplitsOverlay.format(splits[3]/60000F - splits[2]/60000F));
        ...
        if (message.equals("[NPC] Elle: POW! SURELY THAT'S IT! I don't think he has any more in him!")) {
            ...
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) addChatMessage(EnumChatFormatting.AQUA + "Fuel/Stun: " + EnumChatFormatting.RESET + SplitsOverlay.format(splits[4]/60000F - splits[3]/60000F));
        ...
        if (message.contains("DEFEAT") && currentPhase == 4) {
            ...
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) addChatMessage(EnumChatFormatting.AQUA + "Kuudra Kill: " + EnumChatFormatting.RESET + SplitsOverlay.format(splits[5]/60000F - splits[4]/60000F));
            ...
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) mc.thePlayer.addChatMessage(new ChatComponentText(EnumChatFormatting.GRAY + "(Run Overview)").setChatStyle(new ChatStyle().setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ChatComponentText(String.join("\n", overview))))));
            overview = new ArrayList<>();
        ...
        if (message.contains("KUUDRA DOWN") && currentPhase == 4) {
            ...
            CratesOverlay.phase4.add(0F);
            overview.add(EnumChatFormatting.AQUA + "Kuudra Kill: " + EnumChatFormatting.RESET + SplitsOverlay.format(splits[5]/60000F - splits[1]/60000F));
            if (config.getConfig().kuudraCategory.kuudraConfig.showSplitsInChat) {
